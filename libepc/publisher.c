 ...
 * information. The @handler is called on every request matching @key.
 * On this call @publisher, @key and @user_data are passed to the @handler.
 * When replacing or deleting this publication, the function described
 * by @destroy_data is called with @user_data as argument.
 */
void
